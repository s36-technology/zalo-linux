...
function getBinding() {
    ...
    if (process.platform === 'darwin') {
        return require('./profiler_electron1.8_mac.node');
    }

    return createUnsupportedBinding();
}

function createUnsupportedBinding() {
    var profiles = {};
    var unsupported = function() {
        var error = new Error('v8-profiles native binding is not available for this platform.');
        error.code = 'NATIVE_MODULE_NOT_SUPPORTED';
        throw error;
    };
    ...
    return {
        cpu: {
            profiles: profiles,
            startProfiling: unsupported,
            stopProfiling: unsupported,
            setSamplingInterval: unsupported
        }
    };
}
